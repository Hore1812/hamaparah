...
function obtenerHorasMetaTodosEmpleados() {
    global $pdo;
    $sql = "SELECT SUM(horasmeta) as total_horas_meta FROM empleado WHERE activo = 1";
    $sentencia = $pdo->prepare($sql);
    $sentencia->execute();
    $resultado = $sentencia->fetch(PDO::FETCH_ASSOC);
    return $resultado['total_horas_meta'] ?? 0;
}

function obtenerDatosCumplimientoEquipo() {
    global $pdo;
    $empleados = obtenerColaboradores(); 
    $datos_equipo = [];
    ...
    foreach ($empleados as $empleado) {
        ...
        $datos_equipo[] = [
            ...
            'horas_cumplidas' => obtenerHorasAsignadasUsuarioMesActual($id_empleado),
            'horas_meta' => obtenerHorasMetaEmpleado($id_empleado)
        ];
    }
    return $datos_equipo;
}
...
function obtenerHorasCumplidasPorTipoUsuarioMesActual($id_empleado) {
    global $pdo;
    $sql = "
        SELECT 
            cc.tipohora,
            SUM(dh.calculo) AS HorasCumplidas
        ...
        WHERE dh.participante = :id_empleado
          AND l.estado = 'Completo'
...
function obtenerHorasCompletadasPorContratoMesActual($id_empleado) {
    global $pdo;
    $sql = "
        SELECT 
            c.nombrecomercial AS Contrato,
        ...
        HAVING SUM(dh.calculo) > 0
        ORDER BY HorasCompletadas DESC
    ";
    $sentencia = $pdo->prepare($sql);
    $sentencia->bindParam(':id_empleado', $id_empleado, PDO::PARAM_INT);
    $sentencia->execute();
...
function obtenerDatosPlanVsLiquidadoPorContrato($id_empleado = null) {
    global $pdo;
    ...
    if ($id_empleado !== null) {
        ...
        $completed_where_clause = "AND dh.participante = :id_empleado";
    }
    ...
    $sql = "
        WITH PlanData AS (
            ...
            FROM planificacion p
            JOIN contratocliente cc ON p.idContratoCliente = cc.idcontratocli
            WHERE MONTH(p.fechaplan) = MONTH(CURRENT_DATE())
              AND YEAR(p.fechaplan) = YEAR(CURRENT_DATE())
        ),
        CompletedData AS (
            SELECT 
            ...
            WHERE l.estado = 'Completo'
              ...
              AND YEAR(l.fecha) = YEAR(CURRENT_DATE())
              {$completed_where_clause}
            GROUP BY l.idcontratocli
        )
        SELECT 
            c.nombrecomercial AS Contrato,
            COALESCE(pd.horasplan, 0) AS HorasPlanificadas,
            COALESCE(cd.HorasCompletadas, 0) AS HorasCompletadas
        FROM contratocliente cc
        JOIN cliente c ON cc.idcliente = c.idcliente
        LEFT JOIN PlanData pd ON cc.idcontratocli = pd.idcontratocli
        LEFT JOIN CompletedData cd ON cc.idcontratocli = cd.idcontratocli
        WHERE pd.horasplan IS NOT NULL OR cd.HorasCompletadas IS NOT NULL
        ORDER BY HorasPlanificadas DESC, HorasCompletadas DESC
    ";
    ...
    $sql = "SELECT 
            ...
            LEFT JOIN (
                SELECT 
                    ...
                    numeroadenda,
                    fechafin,
                    ROW_NUMBER() OVER(PARTITION BY idcontratocli ORDER BY idadendacli DESC) as rn
                FROM adendacliente
