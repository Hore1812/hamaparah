...
$(document).ready(function() {
    ...
    function renderizarGraficoConsolidado(datos, filtros) {
        ...
        const titleText = `Reporte Consolidado (${cliente_texto} - ${mes_texto} ${filtros.anio}) | Plan: ${formatearNumero(data_consolidada.horas_planificadas, 2)}h | Comp: ${formatearNumero(totalCompletadasGeneral, 2)}h (${cumplimiento.toFixed(2)}%)`;
                    ...
                    datalabels: {
                        ...
                        formatter: (value, context) => {
                            if (totalCompletadasGeneral > 0 && value > 0) {
                                const porcentaje = (value / totalCompletadasGeneral) * 100;
                                return `${formatearNumero(value, 2)}h (${porcentaje.toFixed(2)}%)`;
                            }
                            return value > 0 ? formatearNumero(value, 2) + 'h' : '';
                        }
                    }
                }
            }
